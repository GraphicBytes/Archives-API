...
while true; do
  ...
  3)
    ...
    if [ "$confirmation" != "y" ]; then
      ...
      docker-compose --env-file .env.dev down && docker-compose --env-file .env.dev up -d --build && docker image prune -f
    fi
    ;;
  4)
    echo " "
    ...
    if [ "$confirmation" != "y" ]; then
      ...
      docker-compose --env-file .env.staging down && docker-compose --env-file .env.staging up -d --build && docker image prune -f
    fi
    ;;
  5)
    echo " "
    ...
    if [ "$confirmation" != "y" ]; then
      ...
      docker-compose --env-file .env.production down && docker-compose --env-file .env.production up -d --build && docker image prune -f
    fi
    ;;
  6)
    echo " "
    docker logs archives-api -f
    ;;
  7)
    echo " "
